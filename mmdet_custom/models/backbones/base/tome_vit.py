...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., with_cp=False,
        ...
        if windowed:
            ...
            self.attn = ToMeAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias,
...
class ToMeBlock(Block):
    ...
    """
    def __init__(self, tome_info = {},  **kwargs):
        super().__init__(**kwargs) # inheritant all from Block
...
class ToMeVisionTransformer(BaseModule):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, residual_indices=[], embed_dim=768,
        ...
        if hasattr(self, "dist_token") and self.dist_token is not None:
                self._tome_info["distill_token"] = True

        self.blocks = nn.Sequential(*[
            ToMeBlock(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio,
                  ...
                  use_residual=True if i in residual_indices else False, tome_info = self._tome_info) 
